 ...
 * transactions traitées, voir la mèthode afficherAide().
 *
 * Paramètres 0- site du serveur SQL ("local", "distant" ou "postgres") 1- nom
 * de la BD 2- user id pour �tablir une connexion avec le serveur SQL 3- mot de
...
public class Biblio {
	...
	public static void main(String argv[]) throws Exception {
		...
		try {
			...
			InputStream sourceTransaction = Biblio.class
					.getResourceAsStream("/" + argv[4]);
			try (BufferedReader reader = new BufferedReader(
					new InputStreamReader(sourceTransaction))) {
				gestionBiblio = new GestionBibliotheque(argv[0], argv[1],
						argv[2], argv[3]);
